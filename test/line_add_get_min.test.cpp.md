...
data:
  ...
  bundledCode: "#line 1 \"test/line_add_get_min.test.cpp\"\n#define PROBLEM \"https://judge.yosupo.jp/problem/line_add_get_min\"\
    ...
    \ = v[j++];\n      }\n      for(int i = l; i < r; i++)\n        v[i] = tmp[i];\n\
    \    };\n\n    dc(0, ssize(v), dc);\n\n    return f;\n  }\n}\n\nusing namespace\
    \ algorithm_extend;\n#line 1 \"segtree/Lichao_segment_tree.cpp\"\n//note: size\
    \ should be power of 2, finding min line in default\n\ntemplate<class M>\nstruct\
    \ Lichao_segment_tree {\n  static constexpr M INF = numeric_limits<M>::max();\n\
    \n  vector<array<M, 2>> data;\n  vector<M> xMid;\n  unsigned size;\n\n  Lichao_segment_tree(unsigned\
    \ _size, vector<M> x = vector<M>()) : data(2 * _size, {0, INF}), xMid(2 * _size),\
    ...
    \ ans;\n  }\n\n  void insert_range(int l, int r, M a, M b) {\n    for(l += size,\
    \ r += size; l < r; l >>= 1, r >>= 1) {\n      if (l & 1) insert(l++, a, b);\n\
    ...
    \ a) - pt.begin();\n\n  Lichao_segment_tree<ll> st(bit_ceil((unsigned)q), pt);\n\
    \  for(auto [a, b] : ab)\n    st.insert(1, a, b);\n  for(auto [t, a, b] : query)\
    \ {\n    if (t == 0)\n      st.insert(1, a, b);\n    else\n      cout << st.query(a)\
    \ << '\\n';\n  }\n\n  return 0;\n}\n"
