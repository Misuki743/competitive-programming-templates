...
data:
  ...
  bundledCode: "#line 1 \"test/yuki_2242.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/2242\"\
    ...
    \    return pair(v, prod);\n  }\n};\n#line 5 \"test/yuki_2242.test.cpp\"\n\nint\
    ...
    \ i < n; i++) {\n    argmax[i] = argmax[i - 1];\n    if (t[i] > t[argmax[i]])\n\
    \      argmax[i] = i;\n  }\n\n  vi to(n);\n  for(int i = 0; i < n; i++)\n    to[i]\
    \ = argmax[ranges::upper_bound(h, t[i]) - h.begin() - 1];\n\n  for(int i = 0;\
    \ i < n; i++)\n    if (t[to[i]] <= t[i])\n      to[i] = -1;\n\n  doubling db(18,\
    \ to, vi(n, 1), 0, [](int a, int b) { return a + b; });\n\n  for(auto [a, b] :\
    \ ab) {\n    auto [v, step] = db.jump_while_true(a, [&](int x, int) { return t[x]\
    \ < h[b]; });\n    if (t[a] >= h[b])\n      cout << 1 << '\\n';\n    else if (to[v]\
    \ == -1)\n      cout << -1 << '\\n';\n    else\n      cout << step + 2 << '\\\
    n';\n  }\n\n  return 0;\n}\n"
  ...
  requiredBy: []
  timestamp: '2026-06-10 15:25:39+08:00'
  verificationStatus: TEST_ACCEPTED
  verifiedWith: []
documentation_of: test/yuki_2242.test.cpp
layout: document
redirect_from:
- /verify/test/yuki_2242.test.cpp
- /verify/test/yuki_2242.test.cpp.html
