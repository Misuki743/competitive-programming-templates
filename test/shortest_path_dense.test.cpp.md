...
data:
  ...
  bundledCode: "#line 1 \"test/shortest_path_dense.test.cpp\"\n#define PROBLEM \"\
    ...
    \ ' ' << path[i] << '\\n';\n    }\n  } else {\n    vvc<pair<int, ll>> g(n);\n\
    \    for(int i = 0; i < m; i++) {\n      int u, v, w; cin >> u >> v >> w;\n  \
    \    g[u].emplace_back(v, w);\n    }\n\n    auto [dis, pre] = Dijkstra_sparse(g,\
    \ {s});\n    vi path = recover(pre, t);\n\n    if (path.empty()) {\n      cout\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/shortest_path\"\n\n#include\
    ...
    \ {\n    vc adj(n, vll(n, LLONG_MAX));\n    for(int i = 0; i < m; i++) {\n   \
    \   int u, v, w; cin >> u >> v >> w;\n      adj[u][v] = w;\n    }\n\n    auto\
    \ [dis, pre] = Dijkstra_dense(adj, {s});\n    vi path = recover(pre, t);\n\n \
    \   if (path.empty()) {\n      cout << -1 << '\\n';\n    } else {\n      cout\
    \ << dis[t] << ' ' << ssize(path) - 1 << '\\n';\n      for(int i = 1; i < ssize(path);\
    \ i++)\n        cout << path[i - 1] << ' ' << path[i] << '\\n';\n    }\n  } else\
    \ {\n    vvc<pair<int, ll>> g(n);\n    for(int i = 0; i < m; i++) {\n      int\
    \ u, v, w; cin >> u >> v >> w;\n      g[u].emplace_back(v, w);\n    }\n\n    auto\
    \ [dis, pre] = Dijkstra_sparse(g, {s});\n    vi path = recover(pre, t);\n\n  \
    \  if (path.empty()) {\n      cout << -1 << '\\n';\n    } else {\n      cout <<\
    \ dis[t] << ' ' << ssize(path) - 1 << '\\n';\n      for(int i = 1; i < ssize(path);\
    \ i++)\n        cout << path[i - 1] << ' ' << path[i] << '\\n';\n    }\n  }\n\n\
    \  return 0;\n}\n"
...
layout: document
redirect_from:
- /verify/test/shortest_path_dense.test.cpp
- /verify/test/shortest_path_dense.test.cpp.html
