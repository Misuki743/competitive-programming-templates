...
data:
  ...
  bundledCode: "#line 1 \"numtheory/sieve_of_Eratosthenes.cpp\"\nnamespace sieve_of_Eratosthenes\
    ...
    \      v.resize(old_size * (e + 1));\n      for(int i = old_size; i < ssize(v);\
    \ i++)\n        v[i] = v[i - old_size] * p;\n      for(int d = old_size; d < ssize(v);\
    \ d <<= 1) {\n        for(int i = 0; i + d < ssize(v); i += 2 * d) {\n       \
    \   merge(v.begin() + i, v.begin() + i + d, v.begin() + i + d, v.begin() + min(i\
    \ + 2 * d, (int)size(v)), buf.begin());\n          copy(buf.begin(), buf.begin()\
    \ + min(2 * d, (int)size(v) - i), v.begin() + i);\n        }\n      }\n    });\n\
